        ...
        try:
            ...
            if risk_level == "high":
                ...
                if random.random() < 0.5:  # 50% chance de redirecionar para honeypot vs. bloquear
                    ...
                    # Notificar via todos os canais
                    threat_info = {
                        "level": threat_level,
                        "details": [f"IP {threat_ip} redirecionado para honeypot", "Ameaça de alto risco detectada"]
                    }
                    self.notification_system.notify_threat(threat_ip, threat_info)
                else:
                    ...
                    threat_info = {
                        "level": threat_level,
                        "details": [f"IP {threat_ip} bloqueado automaticamente", "Ameaça de alto risco detectada"]
                    }
                    self.notification_system.notify_threat(threat_ip, threat_info)
                ...
                threat_info = {
                    "level": threat_level,
                    "details": [f"IP {threat_ip} colocado em monitoramento", "Ameaça de risco médio detectada"]
                }
                self.notification_system.notify_threat(threat_ip, threat_info)
